...
func Bin(product string) (string, error) {
	...
	requiresBuildOutput, err := gödelw.run("artifacts", "--absolute", "build", "--requires-build", productBuildID)
	if err != nil {
		return "", err
	}
	...
	binPath, err := gödelw.run("artifacts", "--absolute", "build", productBuildID)
	if err != nil {
		return "", err
	}
	if binPath == "" {
