...
func TestGitHubPublisher(t *testing.T) {
	runPublisherTests(t, publisher.NewGitHubPublisherCreator().Publisher(), true,
		publisherTestCase{
					...
					"foo": {
						...
						Publish: &distgo.PublishConfig{
							...
							PublishInfo: &map[distgo.PublishID]yaml.MapSlice{
								publisher.GitHubPublishTypeName: *mustMapSlicePtr(publisher.GitHubPublishConfig{
									APIURL:     "http://artifactory.domain.com",
									User:       "testUser",
									Token:      "testToken",
									Owner:      "testOwner",
									Repository: "testRepo",
